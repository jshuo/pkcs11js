...
try {
  ...
  const keys = pkcs11Lib.C_GenerateKeyPair(
    ...
    [
      { type: pkcs11.CKA_CLASS, value: pkcs11.CKO_PUBLIC_KEY },
      {
        ...
        value: Buffer.from([0x06, 0x05, 0x2b, 0x81, 0x04, 0x00, 0x0a]),
      }, // secp256k1 curve OID
  ...
  const mechanismb = { mechanism: pkcs11.CKM_ECDSA };

  // Initialize the signature verification process
  pkcs11Lib.C_VerifyInit(session, mechanismb, publicKeyHandle);

  // Perform the verification
  const isVerifiedb = pkcs11Lib.C_Verify(session, sighash, txSignature);

  console.log('Signature Verified:', isVerifiedb); // Should log `true` if the signature is valid
  ...
  pkcs11Lib.C_VerifyInit(session, mechanism, publicKeyHandle);
  const isVerified3 = pkcs11Lib.C_Verify(session, sighash, signatureBuffer);
  console.log('Signature Verified:', isVerified3);
  // Clean up
